...
object MainApplication {
  ...
  def main(args: Array[String]): Unit = {
    ...
    log.info("Null check conf: "+config.getBoolean("test.nullCheck"))
    ...
    if (config.getBoolean("test.absoluteNullCheck")) {
      log.info("Null Check Initiated")
      if (validator.absoluteNullCheck(sourceDataFrame, log)) {
        log.info("Null Check Succeeded")
      } else {
        log.error("Null Check Failed")
      }

...
class Validator {
  ...
  def nullCheck(sourceDataFrame: DataFrame, log: Logger): Boolean = {
    ...
    for (x <- cols) {
      if (sourceDataFrame.where(sourceDataFrame.col(x).isNull).count() >0) {
        log.error(x+" Contains Null Values")
        if (testState)
          testState = false
      }
    }
  ...
  def absoluteNullCheck(sourceDataFrame: DataFrame, log: Logger): Boolean = {
